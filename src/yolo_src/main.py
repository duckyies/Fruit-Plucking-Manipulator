...
class BananaVisionGUI:
    ...
    # ------------------------------------------------------------------
    # Single-frame depth recalculation (used mid-approach)
    # ------------------------------------------------------------------
    def _recalculate_banana_depth(self):
        """
        Grab one fresh frame from the camera, run YOLO, and return the
        banana's (X, Y, Z) in the camera frame if found, else None.

        This is intentionally lightweight — no averaging, just one frame —
        because we only use it to catch grossly wrong initial depth readings.
        """
        with frame_lock:
            frame   = None if latest_frame is None else latest_frame.copy()
            depth_f = latest_depth_frame

        if frame is None or depth_f is None:
            return None
        ...
        for box in results.boxes:
            ...
            if not is_unripe or conf < self.MIN_UNRIPE_CONF:
                continue
            ...
            return X, Y, -Z   # same sign convention as update_camera
    ...
    def go_search(self):
        ...
        def task():
            # 🔴 Disable detection while moving to search pose
            self.search_active = False

            self.movel(SEARCH_POSX, v=[15, 15], a=[10, 10],
                       ref=self.base, mod=self.abs_mode)

            # 🟢 Enable detection AFTER reaching search pose
            self.scanning = True
    ...
    def go_rest(self):
        self.scanning       = False
        self.collecting     = False
        self.search_active  = False   # 🔴 Disable detection
        self.banana_readings = []
        self.is_moving      = True
        ...
        def task():
            try:
                ...
                while True:
                    ...
                    if dist_mm <= (STEP_MM + 150):
                        ...
                        if approach_dist > 1.0:          # only move if there's meaningful distance left
                            ...
                            self.movel([fx, fy, fz] + orient, v=STEP_V, a=STEP_A,
                                    ref=self.base, mod=self.abs_mode)
                        final_joints = self.normalize_joints(self.get_current_posj())
                        final_joints[1] -= 3.0
                        self.movej(self.posj(*final_joints), vel=SCAN_SPEED, acc=SCAN_ACC)
                        break
                    ...
                    # ── Post-step depth recalculation ──────────────────────
                    # Grab one fresh frame and check whether the banana is
                    # actually closer than our current target suggests.
                    recalc = self._recalculate_banana_depth()
                    if recalc is not None:
                        rX, rY, rZ = recalc
                        new_tx, new_ty, new_tz = self.camera_to_base(rX, rY, rZ)
                        new_dist = math.sqrt((new_tx - cx)**2 +
                                             (new_ty - cy)**2 +
                                             (new_tz - cz)**2)
                        old_dist = math.sqrt((tx - cx)**2 +
                                             (ty - cy)**2 +
                                             (tz - cz)**2)
                        # Only accept the new target if it is meaningfully
                        # closer (>80 mm) — avoids flip-flopping on noise.
                        if new_dist < old_dist - 80.0:
                            print(f"  🔄 Depth corrected: {old_dist:.1f} mm → {new_dist:.1f} mm")
                            tx, ty, tz = new_tx, new_ty, new_tz
                        else:
                            print(f"  ✔ Depth check OK (recalc={new_dist:.1f} mm, target={old_dist:.1f} mm)")
                    else:
                        print("  ⚠️  Depth recalc: banana not visible this frame, keeping current target")
        ...
        if frame is not None and depth_f is not None:
            ...
            for box in results.boxes:
                ...
                if is_unripe and conf >= self.MIN_UNRIPE_CONF and not self.target_locked:
                    banana_this_frame = (X, Y, -Z)  
                    break

            # ── Collection-phase state machine ──────────────────────────────
            if banana_this_frame is not None and not self.is_moving and self.search_active and not self.target_locked:
